...
' save the screen xy cordinates
    ...
    Else
     c = vbBlack
    End If
' test marker of each corner
    Picture1.Circle (pntsAry(cnt).x, pntsAry(cnt).y), 4, c
    cnt = cnt + 1
   Next z
  Next y
 Next x

 tStr = "122443313715264856688775" ' 8 points of the cube
 For i = 1 To Len(tStr) Step 2
' get corner 1 and corner 2
  x = Val(Mid(tStr, i, 1)) - 1
  y = Val(Mid(tStr, i + 1, 1)) - 1
  If i < 9 Then
   c = RGB(0, 0, 255)
  Else
   c = vbBlack
  End If
' draw line from corner 1 to corner 2
  Picture1.Line (pntsAry(x).x, pntsAry(x).y)-(pntsAry(y).x, pntsAry(y).y), c
 Next i
